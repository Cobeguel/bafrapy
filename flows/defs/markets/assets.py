...
from flows.task_tracking import track_dagster_task
...
def sync_exchange_markets(
    ...
    description = f"SYNC Markets {exchange_id}"

    with track_dagster_task(repository, run_id=context.run_id, description=description):
        try:
        ...
        with repository.start_session() as repo:
            ...
            context.log.info(
                f"Synced {len(client_markets)} markets for {exchange_id}; marked {unlisted_count} as unlisted"
            )

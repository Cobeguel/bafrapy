...
class Provider(Base):
    ...
    status: Mapped[str] = mapped_column(
        String(255), nullable=False, server_default=text("'ACTIVE'")
    )
    display_name: Mapped[Optional[str]] = mapped_column(String(255))
    external_name: Mapped[Optional[str]] = mapped_column(String(255))

    resolutions: Mapped[list["Resolution"]] = relationship(
        "Resolution", secondary="providers_resolutions", back_populates="providers"
    )
...
class Resolution(Base):
    ...
    __table_args__ = (Index("resolutions_seconds_unique", "seconds", unique=True),)
    ...
    providers: Mapped[list["Provider"]] = relationship(
        "Provider", secondary="providers_resolutions", back_populates="resolutions"
    )
...
class Asset(Base):
    ...
    provider: Mapped[Optional["Provider"]] = relationship(
        "Provider", back_populates="assets"
    )
...
t_providers_resolutions = Table(
    ...
    ForeignKeyConstraint(
        ...
        ["providers.id"],
        ondelete="SET NULL",
        name="providers_resolutions_providers_id_foreign",
    ),
    ForeignKeyConstraint(
        ["resolutions_id"],
        ["resolutions.id"],
        ondelete="SET NULL",
        name="providers_resolutions_resolutions_id_foreign",
    ),

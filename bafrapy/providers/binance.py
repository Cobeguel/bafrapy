...
class BinanceConfig:
    ...
    def data_monthly_URL(self, symbol: str, resolution: str, year: int, month: int):
        return (
            URL(self.data_vision_domain)
            / self.data_monthly_klines_URL
            / symbol
            / resolution
            / f"{symbol}-{resolution}-{year}-{month:02d}.zip"
        )

    def data_daily_URL(
        self, symbol: str, resolution: str, year: int, month: int, day: int
    ):
        return (
            URL(self.data_vision_domain)
            / self.data_daily_klines_URL
            / symbol
            / resolution
            / f"{symbol}-{resolution}-{year}-{month:02d}-{day:02d}.zip"
        )
...
class BinanceProvider(ProviderClient):
    ...
    def _process_CSV_OHLCV(
        self, bytes: bytes, symbol: str, resolution: int
    ) -> pd.DataFrame:
        with io.BytesIO(bytes) as zip_bytes:
            with zipfile.ZipFile(zip_bytes) as zf:
                if len(zf.namelist()) != 1:
                    raise ValueError(
                        "Binance provider expected exactly one file in the ZIP archive"
                    )
                csv_file_name = zf.namelist()[0]
                if not csv_file_name.endswith(".csv"):
                    raise ValueError(
                        "Binance provider expected exactly one CSV file in the ZIP archive"
                    )
                ...
                with zf.open(csv_file_name) as csv_file:
                    ...
                    df["time"] = pd.to_datetime(
                        df["time"].apply(normalize_mixed_timestamp), utc=True
                    )
    ...
    def list_available_symbols(self) -> List[SymbolClient]:
        ...
        response = self._api_http_client.request(
            self._config.symbols_URL(), HTTPMethod.GET
        )
        body = response.json()
        log().info(
            f"Fetched available symbols from {self._provider_name}",
            LogField(key="symbols", value=len(body["symbols"])),
        )
        ...
        for item in body["symbols"]:
            ...
            try:
                ...
                log().warning(
                    "Not found filters. They will not be stored",
                    LogField(key="provider", value=self._provider_name),
    ...
    def symbol_first_date(self, symbol: str) -> datetime:
        log().info(
            "Getting first date",
            LogField(key="provider", value=self._provider_name),
            LogField(key="symbol", value=symbol),
        )
        params = {"symbol": symbol, "fromId": 0, "limit": 1}
        response = self._api_http_client.request(
            self._config.aggTrades_URL(), "GET", params=params
        )
        body = response.json()
        ...
        log().debug(
            "First date for symbol",
            LogField(key="symbol", value=symbol),
            LogField(key="date", value=first_date),
        )
        return first_date
    ...
    def symbol_last_date(self, symbol: str) -> date:
        ...
        response = self._api_http_client.request(
            self._config.aggTrades_URL(), HTTPMethod.GET, params=params
        )
        ...
        while not found and current_attempt < self._config.last_day_gaps_attempts:
            response = self._data_http_client.request(
                self._config.data_daily_URL(
                    symbol, "1m", last_date.year, last_date.month, last_date.day
                ),
                "HEAD",
                retrayable=False,
                raisable=False,
        ...
        if not found:
            log().error(
                f"Last date for symbol {symbol} not found",
                LogField(key="symbol", value=symbol),
                LogField(key="last date checked", value=last_date),
            )
            raise ValueError(f"Last date for symbol {symbol} not found")

        log().debug(
            "Last date for symbol",
            LogField(key="symbol", value=symbol),
            LogField(key="date", value=last_date),
        )
    ...
    def get_day_data(
        ...
        log().info(
            ...
            LogField(key="provider", value=self._provider_name),
            LogField(key="symbol", value=symbol),
            LogField(key="day", value=day),
            LogField(key="resolution", value=resolution.name),
        )
        response = self._data_http_client.request(
            self._config.data_daily_URL(
                symbol, resolution.name, day.year, day.month, day.day
            ),
            HTTPMethod.GET,
        )
    ...
    def get_month_data(
        ...
        log().info(
            ...
            LogField(key="provider", value=self._provider_name),
            LogField(key="symbol", value=symbol),
            LogField(key="month", value=month),
            LogField(key="resolution", value=resolution.name),
        )
        response = self._data_http_client.request(
            self._config.data_monthly_URL(
                symbol, resolution.name, month.year, month.month
            ),
            HTTPMethod.GET,
        )
        data = self._process_CSV_OHLCV(response.content, symbol, resolution.seconds)

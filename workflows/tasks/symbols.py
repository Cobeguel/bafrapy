...
class SymbolTask:
    ...
    def run(self, providerName: str):
        ...
        log().info(
            "Symbols fetched", LogField(key="count", value=len(provider_symbols))
        )
        ...
        with self.repository.start_session() as repo:
            ...
            new_symbols = [
                s for s in provider_symbols if s.symbol not in assets_symbols
            ]
            new_assets = [
                Asset(
                    id=str(uuid4()),
            ...
            log().info(
                "Adding new assets to repository",
                LogField(key="assets", value=len(new_assets)),
            )
            ...
            log().info(
                "Undated assets count",
                LogField(key="count", value=undated_assets_count),
            )
            for i in range(0, undated_assets_count, 100):
                ...
                for asset in undated_assets:
                    if asset.symbol is None:
                        log().warning(
                            "Asset has no symbol. Will not be updated",
                            LogField(key="asset", value=asset.id),
                        )
                        continue
                ...
                repo.commit()
                log().info(
                    "Processed symbol batch",
                    LogField(key="count", value=len(undated_assets)),
                )

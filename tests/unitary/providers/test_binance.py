...
class TestBinanceConfig:
    ...
    def setup_config(self):
        config = BinanceConfig(
            ...
            api_backoff=BackoffConfig(
                timeout=10, max_tries=3, backoff_factor=1, giveup_codes=[404]
            ),
    ...
    def test_data_daily_URL(self):
        assert (
            str(self.config.data_daily_URL("BTCUSDT", "1m", 2024, 5, 25))
            == "https://data.binance.vision/data/spot/daily/klines/BTCUSDT/1m/BTCUSDT-1m-2024-05-25.zip"
        )

    def test_data_monthly_URL(self):
        assert (
            str(self.config.data_monthly_URL("BTCUSDT", "1m", 2024, 5))
            == "https://data.binance.vision/data/spot/monthly/klines/BTCUSDT/1m/BTCUSDT-1m-2024-05.zip"
        )

    def test_symbols_URL(self):
        assert (
            str(self.config.symbols_URL())
            == "https://api.binance.com/api/v3/exchangeInfo"
        )

    def test_aggTrades_URL(self):
        assert (
            str(self.config.aggTrades_URL())
...
class TestBinanceProvider:
    ...
    def test_symbol_first_date(self):
        responses.add(
            ...
            match=[
                matchers.query_param_matcher(
                    {"symbol": "BTCUSDC", "fromId": "0", "limit": "1"}
                )
            ],
            json=[
                {
        ...
        assert self.provider.symbol_first_date("BTCUSDC") == datetime.fromtimestamp(
            1662076800796 / 1000
        )
    ...
    def test_symbol_last_date_same_day(self):
        responses.add(
            ...
            match=[matchers.query_param_matcher({"symbol": "BTCUSDC", "limit": "1"})],
            json=[
                {
    ...
    def test_download_day_data(self):
        ...
        df_result = self.provider.get_day_data(
            "BAFRAPY", datetime(2024, 5, 25).date(), ResolutionClient("1m", 60)
        )
        ...
        for index, row in df_result.iterrows():
            ...
            assert (
                row["time"].date()
                == normalize_mixed_timestamp(kline_rows[index]["open_time"]).date()
            )
